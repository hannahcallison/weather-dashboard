...
$("form").submit(function (event) {
  ...
  // } else {
    ...
    for (let i = 0; i < cities.length; i++) {
      searchHistory.append(`<button class="resultBtn">${cities[i]}</button>`)
    }
    $(".resultBtn").on("click", function (event) {
      event.preventDefault();
      var recalledCity = $(this).text()
      cityWeather(recalledCity)
    })
})
...
// Fix closing tags
// Same city if statement.. in store cities search 